...
public class LLVMGeneratorCopy {
    private ArrayList<IASTNode> stack = new ArrayList<>();
    private HashMap<String,IASTNode> global = new HashMap<>();
    private int currentLevel = 0;
    private static final LLVMGeneratorCopy instance = new LLVMGeneratorCopy();

    private int regId = 0;


    public static LLVMGeneratorCopy getInstance() {
        return instance;
    }
...
//        expAST.setRegID(regId++);
    ...
    private void OutPutAddExp(AddExpAST addExpAST){
        ArrayList<AddExpAST> addExpASTS = new ArrayList<>();
        AddExpAST tmpAddExpAST = addExpAST;
        addExpASTS.add(tmpAddExpAST);
        while (tmpAddExpAST.getAddExpAST()!=null){
            addExpASTS.add(tmpAddExpAST.getAddExpAST());
            tmpAddExpAST = tmpAddExpAST.getAddExpAST();
        }
        if(currentLevel>0){
            for(int i=0;i<addExpASTS.size();i++){
                OutPutMulExp(addExpASTS.get(i).getMulExpAST());
                addExpASTS.get(i).setRegID(addExpASTS.get(i).getMulExpAST().getRegID());
                addExpASTS.get(i).setReturnValue(addExpASTS.get(i).getMulExpAST().getReturnValue());
                addExpASTS.get(i).setReturnType(addExpASTS.get(i).getMulExpAST().getReturnType());
            }
            for(int i=0;i<addExpASTS.size()-1;i++){
                AddExpAST addExpASTLeft = addExpASTS.get(i);
                AddExpAST addExpASTRight = addExpASTS.get(i+1);

                String tmp = addExpASTLeft.getPrintLLVM();
                addExpAST.setRegID(regId++);
                addExpAST.setReturnType("int");
                addExpAST.setReturnValue("");
                System.out.println("    " +  addExpAST.getPrintLLVM()+ " = "+ Operator(addExpASTLeft.getOp()) + " i32 " +
                        tmp + ", " + addExpASTRight.getPrintLLVM());
            }
        }else{
            for(int i=0;i<addExpASTS.size();i++){
                OutPutMulExp(addExpASTS.get(i).getMulExpAST());
    ...
    private void OutPutMulExp(MulExpAST mulExpAST){
        ArrayList<MulExpAST> mulExpASTS = new ArrayList<>();
        MulExpAST tmpMulExpAST = mulExpAST;
        mulExpASTS.add(tmpMulExpAST);
        while (tmpMulExpAST.getMulExpAST()!=null){
            mulExpASTS.add(tmpMulExpAST.getMulExpAST());
            tmpMulExpAST = tmpMulExpAST.getMulExpAST();
        }
        if(currentLevel>0){
            for(int i=0;i<mulExpASTS.size();i++){
                OutPutUnaryExp(mulExpASTS.get(i).getUnaryExpAST());
                mulExpASTS.get(i).setRegID(mulExpASTS.get(i).getUnaryExpAST().getRegID());
                mulExpASTS.get(i).setReturnValue(mulExpASTS.get(i).getUnaryExpAST().getReturnValue());
                mulExpASTS.get(i).setReturnType(mulExpASTS.get(i).getUnaryExpAST().getReturnType());
            }
            for(int i=0;i<mulExpASTS.size()-1;i++){
                MulExpAST mulExpASTLeft = mulExpASTS.get(i);
                MulExpAST mulExpASTRight = mulExpASTS.get(i+1);
                String tmp = mulExpASTLeft.getPrintLLVM();
                mulExpAST.setRegID(regId++);
                mulExpAST.setReturnType("int");
                mulExpAST.setReturnValue("");
                System.out.println("    " +  mulExpAST.getPrintLLVM()+ " = "+ Operator(mulExpASTLeft.getOp()) + " i32 " +
                        tmp + ", " + mulExpASTRight.getPrintLLVM());
            }
        }else{
            for(int i=0;i<mulExpASTS.size();i++) {
...
//            unaryExpAST.globalIdent = primaryExpAST.globalIdent;
        ...
        else if(unaryExpAST.getUnaryOpAST()!=null)
        {
            if(currentLevel>0){
                ...
                System.out.println( "    " +unaryExpAST.getPrintLLVM() + " = " + Operator(unaryExpAST.getUnaryOpAST().getOperator()) + " i32 0, "
